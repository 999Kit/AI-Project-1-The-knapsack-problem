...
def branch_and_bound(problem: KnapsackProblem):
    ...
    while len(pq) > 0:
        ...
        final_state_count = 0  # to keep track whether any left or right node in the last level is valid
        # if lower bound is larger than upper bound then we prune
        if current_level + 1 == size:
            # if current ub is smaller (better) than final_ub
            if len(class_set) == problem.m and left_lb != 1 and left_ub < final_ub:
                final_state_count += 1
                final_ub = left_ub

                # update current path
                i = size - 1 - (current_level + 1 - 1)
                left_state = cur_state | (1 << i)

                # convert integer to binary string, remove '0b' prefix, and pad with zeros to length 'size'
                binary_string = bin(left_state)[2:].zfill(size)
        ...
        if current_level + 1 == size:
            # if current ub is smaller (better) than final_ub
            if len(right_class_set) == problem.m and right_ub < final_ub:
                final_state_count += 1
                final_ub = right_ub

                # update current path
                i = size - 1 - (current_level + 1 - 1)
                right_state = cur_state & ~(1 << i)  # clear the bit at current_level - 1

                # convert integer to binary string, remove '0b' prefix, and pad with zeros to length 'size'
                binary_string = bin(right_state)[2:].zfill(size)
